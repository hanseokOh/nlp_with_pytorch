...
'''
use python wrapper to easily use CTK(Champollion Toolkit) 
https://github.com/LowResourceLanguages/champollion
'''

import sys, argparse, os

CTK_ROOT = 'C:/Users/rnfek/hanseok/champollion/' # champollion 깃헙 레포지토리 클론 위치
BIN = CTK_ROOT + '/bin/champollion.EC_utf8' # CTK_ROOT에 CTK 위치를 지정
CMD = "%s -c %f -d %s %s %s %s"
OMIT = "omitted"
DIR_PATH = './tmp/'
...
if __name__=="__main__":
    ...
    cmd = CMD % (BIN, config.ratio, config.dict, config.src_ref, config.tgt_ref,
                INTERMEDIATE_FN)
    os.system(cmd) # CMD 명령어를 코드 상에서 입력하는 방법 익혀두기
